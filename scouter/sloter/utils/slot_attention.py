...
class SlotAttention(nn.Module):
    ...
    def forward(self, inputs, inputs_x, save_id=None, sens=-1):
        b, n, d = inputs.shape
        ...
        for _ in range(self.iters):
            ...
            dots = torch.div(dots, dots.sum(2).expand_as(dots.permute([2, 0, 1])).permute([1, 2, 0])) * dots.sum(2).sum(
                1
            ).expand_as(dots.permute([1, 2, 0])).permute(
                [2, 0, 1]
            )  # * 10
        ...
        if self.vis or save_id or sens > -1:
            ...
            slots_vis = ((slots_vis - slots_vis.min()) / (slots_vis.max() - slots_vis.min()) * 255.0).reshape(
